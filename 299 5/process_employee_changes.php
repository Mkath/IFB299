...
//Matches each field to it's respective pattern. If they do not match then an error is created
  ...
  if ($errors == null){
		...
		//builds are connection to the database and updates all the fields with the new information.
          ...
          $stmt = $conn->prepare("UPDATE `employee_details` SET `employee_firstname` = :firstname, `employee_phone` = :phone, `employee_postal` = :postal, `employee_dob` = :dob, `employee_lastname` = :lastname, `employee_email` = :email WHERE `employeeid` = :eid");
